...
public class VisionLookUpTable {
    ...
    public VisionLookUpTable() {
        ...
        shooterConfig.getShooterConfigs().add(new ShooterPreset(35.85, 70, 50, 4.5)); // Distance -> Bumper
        shooterConfig.getShooterConfigs().add(new ShooterPreset(36.7, 70, 50, 5)); // Distance -> Bumper
        shooterConfig.getShooterConfigs().add(new ShooterPreset(38.5, 70, 50, 5.5)); // Distance -> Bumper
    ...
    /*
     * Perform fast binary search to find a matching shooter preset. if no matching
     * preset is found it interpolates a
     ...
     */
    private ShooterPreset binarySearchDistance(List<ShooterPreset> ShooterConfigs, int StartIndex, int EndIndex,
            double Distance) {
        ...
        if (EndIndex - StartIndex == 1) {
            ...
            return interpolateShooterPreset(shooterConfig.getShooterConfigs().get(StartIndex),
                    shooterConfig.getShooterConfigs().get(EndIndex), percentIn);
        }
    ...
    /*
     * Obtain a new shooter preset by interpolating between two existing shooter
     * presets
     * 
     ...
     * 
     * @param PercentIn: Amount of percentage between the two values the new preset
     * needs to be
     * 
     * @return new interpolated shooter preset
     */
    private ShooterPreset interpolateShooterPreset(ShooterPreset StartPreset, ShooterPreset EndPreset,
            double PercentIn) {
        double armAngle = StartPreset.getArmAngle() + (EndPreset.getArmAngle() - StartPreset.getArmAngle()) * PercentIn;
        double leftShooter = StartPreset.getLeftShooter()
                + (EndPreset.getLeftShooter() - StartPreset.getLeftShooter()) * PercentIn;
        double rightShooter = StartPreset.getRightShooter()
                + (EndPreset.getRightShooter() - StartPreset.getRightShooter()) * PercentIn;
